...
def trim_data_for_provisions(data, row_labels, column_labels):
    first_state_in_row_labels = text_files.state_and_provision_number_from_label(
        row_labels[0])[0]
    first_state_row_labels = list(filter(
        lambda label: text_files.state_and_provision_number_from_label(label)[0] == first_state_in_row_labels,
        row_labels))
    last_state_in_column_labels = text_files.state_and_provision_number_from_label(
        column_labels[-1])[0]
    last_state_column_labels = list(filter(
        lambda label: text_files.state_and_provision_number_from_label(label)[0] == last_state_in_column_labels,
        column_labels))
...
def provision_numbers_from_labels(labels):
    return list(map(
        lambda label: text_files.state_and_provision_number_from_label(label)[1],
        labels))
...
def unique_states_from_labels(labels):
    unique_states = []
    for label in labels:
        state = text_files.state_and_provision_number_from_label(label)[0]
